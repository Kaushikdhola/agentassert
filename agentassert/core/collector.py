"""Test collector for discovering AgentAssert tests.
...
class TestCollector:
    """
    Discovers and collects AgentAssert test items.
    ...
    def _load_conftest(self, conftest_path: Path) -> None:
        ...
        for name in dir(module):
            ...
            if callable(obj) and hasattr(obj, "__agentassert_fixture__"):
    ...
    def _collect_from_file(self, file_path: Path) -> None:
        ...
        for func_name, line_number in test_functions:
            ...
            if not hasattr(func, "__agentassert_test__"):
                continue

            # Check for scenarios
            scenarios = getattr(func, "__agentassert_scenarios__", None)
            if scenarios:
